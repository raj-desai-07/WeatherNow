...
def home(request):
        ...
        for item in forecast_data["list"]:
            ...
            if start_time <= forecast_time <= end_time:
                ...
                if weather_main == "Clear":
                    icon = "fa-moon indigo-fc" if is_night else "fa-sun yellow-fc"

                elif weather_main == "Clouds":
                    icon = "fa-cloud-moon indigo-fc" if is_night else "fa-cloud-sun yellow-fc"
    ...
    if data:
        ...
        # FIX FOR PYTHONANYWHERE TIMEZONE ISSUE
        timezone_offset = weather["timezone"]

        sunrise_time = datetime.utcfromtimestamp(
            weather["sys"]["sunrise"] + timezone_offset
        ).strftime("%I:%M %p")

        sunset_time = datetime.utcfromtimestamp(
            weather["sys"]["sunset"] + timezone_offset
        ).strftime("%I:%M %p")

        result = {
            "city": data[0]["name"],
            "country": pycountry.countries.get(alpha_2=data[0]["country"]).name,
